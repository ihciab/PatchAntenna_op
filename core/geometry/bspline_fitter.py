"""
B样条轮廓拟合模块
原代码位置：Rebuild/BSplineContour.py
...
重构说明：
- 将原 BSplineContour 类重构为 BSplineFitter 类
- 保持原有算法和逻辑完全不变
- 更新导入路径以适配新结构
"""
import cv2
...
class BSplineFitter:
    """
    B样条轮廓拟合类
    原类：Rebuild/BSplineContour.py 中的 BSplineContour
    
    用于对轮廓进行B样条拟合，保存拟合轮廓的点和轮廓之间的嵌套关系
    """
    def __init__(self, img, edges, lr=0.1, delta = 2, step = 30, threshold:float=10.0, show=True, save=''):
        ...
        for i, contour in enumerate(self.__contours):
            ...
            if i % 2 == 1:
                ...
                while step>1:
                    ...
                    # 原代码修复：确保len_control至少为2，且k不超过len_control-1
                    if len_control < 2:
                        ...
                        if step > 1:
                            ...
                            print(f"警告: 轮廓 {i} 的控制点数量不足（{len_control}），跳过B样条拟合")
                            break
                    
                    # 确定样条阶数k（确保k < 数据点数量）
                    k = 3 if len_contour > 3 else len_contour - 1
                    # 原代码修复：确保k不超过len_control-1
                    k = min(k, len_control - 1)
                    
                    # 确保k至少为1
                    if k < 1:
                        print(f"警告: 轮廓 {i} 的样条阶数k={k}无效，跳过B样条拟合")
                        break
