...
- 数值只保留数字部分，不要包含单位"""
        ...
        # 原代码：解析API返回结果（兼容新旧两种格式）
        try:
            # 尝试新格式 structure_size
            structure_size_match = re.search(r"structure_size\s*=\s*(\{.*?})", api_result, re.DOTALL)
            if not structure_size_match:
                # 兼容旧格式 fss_size
                structure_size_match = re.search(r"fss_size\s*=\s*(\{.*?})", api_result, re.DOTALL)
            
            if structure_size_match:
                structure_size = eval(structure_size_match.group(1))
                # 验证必须包含X、Y、Z、Unit字段
                required_fields = ['X', 'Y', 'Z', 'Unit']
                if all(field in structure_size for field in required_fields):
                    return structure_size
                else:
                    print(f"警告：返回字典缺少必需字段，当前字段：{list(structure_size.keys())}")
                    # 补充缺失字段
                    for field in required_fields:
                        if field not in structure_size:
                            structure_size[field] = 'unknown'
                    return structure_size
            else:
                print("未找到structure_size或fss_size字典")
                print(f"API返回内容：\n{api_result}")
                return {}
    ...
    def select_structure_image(self, encoded_images: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """
        使用大模型逐个判断并选择结构图（支持FSS、超材料、metasurface等）
        ...
        """
        # 原代码：图像选择提示词（扩展支持多种结构类型）
        select_prompt = """请判断这张图像是否为结构图（FSS、超材料、metasurface等）。

判断标准（必须同时满足）:
✓ 必须包含：箭头、英文长度参数（如 L、W、P、a、b、g、h、r等）
✓ 必须显示：几何结构（如方形环、十字形、圆环、贴片、谐振器等）
            ...
            if api_result:
                try:
                    ...
                    if json_match:
                        ...
                        # 兼容新旧两种返回格式
                        is_structure = result_json.get("is_structure", result_json.get("is_fss_structure", False))
                        reason = result_json.get("reason", "无理由")
                        
                        print(f"  判断结果: {'✓ 是结构图' if is_structure else '✗ 不是结构图'}")
                        print(f"  理由: {reason}")
                        
                        if is_structure:
                            print(f"\n>>> 已选择结构图：页{img_info['page_num']}, 索引{img_info['img_index']}")
                            return img_info
                except Exception as e:
                    print(f"  解析API结果错误: {e}")
        
        print("\n!!! 警告：未找到符合标准的结构图（FSS/超材料/metasurface）")
        return None
    
    def analyze_color_material(self, selected_image: Dict[str, Any]) -> Tuple[Dict[str, str], Dict[str, str], Dict[str, str]]:
        """
        使用大模型分析结构图的颜色和材料（支持FSS、超材料、metasurface等）
        
        原函数：Rebuild/Fss_analyzer.py 中 analyze_fss_pdf 函数的颜色材料分析部分
        
        参数:
            selected_image: 选中的结构图信息
        ...
        # 原代码：颜色材料分析提示词（扩展支持多种结构类型）
        color_material_analysis_prompt = f"""请对提供的结构图（FSS、超材料、metasurface等）进行颜色和材料分析。
...
3. 组件描述应基于结构特点（FSS、超材料、metasurface等）：
   - 辐射单元/谐振单元 (radiating/resonant element)
   - 接地层 (ground plane)
   - 介质基板 (dielectric substrate)
   - 等效传输线 (equivalent transmission line)
   - 金属贴片 (metal patch)
   - 超材料单元 (metamaterial unit)
   等
...
- HSV范围必须严格对应标准定义"""
    ...
    def analyze_structure_document(self, document_path: str, output_folder: str = r"structure_out") -> Tuple[Dict[str, str], Dict[str, str], str]:
        """
        分析结构文档（主入口函数，支持FSS、超材料、metasurface等）
        ...
        返回:
            (col_mats, structure_size, pic_name) 元组
            - col_mats: 颜色-材料映射字典
            - structure_size: 结构周期参数字典
            - pic_name: 保存的结构图文件名
        """
        ...
        # 2. 先进行文本分析，提取结构周期参数（X、Y、Z）（原代码逻辑）
        structure_size = self.extract_structure_parameters_from_text(full_text)
        
        print("\n" + "=" * 50)
        print("结构周期参数提取完成")
        print("=" * 50)
        print(f"提取结果: {json.dumps(structure_size, indent=2, ensure_ascii=False)}")
        ...
        # 4. 使用大模型逐个判断并选择结构图（原代码逻辑）
        print("\n" + "=" * 50)
        print("开始逐个判断图像...")
        print("=" * 50)
        
        selected_image = self.select_structure_image(encoded_images)
        
        if not selected_image:
            print("\n!!! 错误：该文档中未找到符合标准的结构图（FSS/超材料/metasurface）")
            return {}, structure_size, ""
        ...
        output_path = os.path.join(
            output_folder,
            f"structure_p{selected_image['page_num']}_i{selected_image['img_index']}.png",
        )
        pic_name = f"structure_p{selected_image['page_num']}_i{selected_image['img_index']}.png"
        selected_image["image"].save(output_path)
        print(f"\n已保存结构图: {output_path}")
        
        # 6. 调用大模型进行颜色和材料分析（原代码逻辑）
        print("\n" + "=" * 50)
        print("正在调用API分析结构图颜色和材料...")
        print("=" * 50)
        
        color_ranges, col_mats, col_comps = self.analyze_color_material(selected_image)
        
        # 原代码：返回 col_mats, structure_size, pic_name
        return col_mats, structure_size, pic_name


# 向后兼容的类名和函数（保持原有接口）
FSSAnalyzer = StructureAnalyzer


def analyze_fss_pdf(pdf_path: str, output_folder: str = r"fss_out") -> Tuple[Dict[str, str], Dict[str, str], str]:
    """
    向后兼容函数：分析FSS PDF文档（内部调用新的通用分析函数）
    ...
    # 使用新的通用函数，但保持输出文件夹为 fss_out 以兼容旧代码
    return analyzer.analyze_structure_document(pdf_path, output_folder)


def analyze_structure_pdf(pdf_path: str, output_folder: str = r"structure_out") -> Tuple[Dict[str, str], Dict[str, str], str]:
    """
    新的通用函数：分析结构PDF文档（支持FSS、超材料、metasurface等）
